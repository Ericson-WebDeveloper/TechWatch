...
const CheckOut = (props: CheckOutProps) => {
    const [stripe, setStripe] = useState<stripecard>({
        stripe: null, stripeToken: null,
        cardElement: null,
        processing: false,
    ...
    const setUpStripeCard = async () => {
        ...
        stripe.stripe = await loadStripe(configEnv.stripe_sk_key);
    ...
    useEffect(() => {
        if (stripe.cardElement && divRef.current) {
            try {
                stripe.cardElement.mount(divRef.current);
                ...
                setStripe((prev) => ({ ...prev, 'card_error': 'payment with card not available' }));
                setStripe((prev) => ({ ...prev, 'processing': false }));
            }
        ...
        return () => {
            if (stripe.cardElement) {
                stripe.cardElement.unmount();
            }
        };
    }, [stripe.cardElement, divRef.current]); //stripe
