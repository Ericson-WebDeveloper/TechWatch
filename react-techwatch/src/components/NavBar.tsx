...
const NavBar = (props: NavBarProps) => {
    ...
    const [showCartDropDown, setCartDropDown] = useState(false);
    ...
    return (
        ...
        text-gray-200 shadow-lg navbar navbar-expand-lg navbar-light">
                {/*  */}
                <div className="container-fluid w-full flex flex-wrap items-center justify-between px-6 static">

                    <button onClick={() => setSHowNavBar(!showNavBar)} className="navbar-toggler text-white border-0 hover:shadow-none hover:no-underline py-2 px-2.5 bg-transparent 
                    ...
                    {/* <!-- Left links --> */}
                    <ul className="navbar-nav flex flex-col pl-0 list-style-none mr-auto hidden md:flex">
                    ...
                    <div className="flex items-center relative">
                        ...
                        <div className="dropdown relative">
                            <span onClick={() => setCartDropDown(!showCartDropDown)} className="text-white opacity-60 hover:opacity-80 focus:opacity-80 mr-4 dropdown-toggle hidden-arrow flex items-center"
                            ...
                            <ul className={`dropdown-menu min-w-max absolute bg-white text-base z-50 float-left py-2 list-none 
                            text-left rounded-lg shadow-lg mt-1 m-0 bg-clip-padding border-none left-auto right-0 ${showCartDropDown ? 'block' : 'hidden'}`}
                                aria-labelledby="dropdownMenuButton1">
                                <li>
                                    <Link to="/cart" onClick={() => setCartDropDown(false)} className="dropdown-item text-sm py-2 px-4 font-normal block w-full whitespace-nowrap bg-transparent 
                        ...
                        </div>
                        {
                            !user && !token
                                ...
                                <>
                                    <div className="dropdown relative">
                                        <span onClick={() => setProfileDropDown(!showProfileDropDown)} className="dropdown-toggle flex items-center hidden-arrow"
                        ...
                        shadow-lg mt-1 m-0 bg-clip-padding border-none left-auto right-0 ${showProfileDropDown ? 'block' : 'hidden'}`} aria-labelledby="dropdownMenuButton2">
                                            {/* <li>
                                                <Link onClick={() => setProfileDropDown(!showProfileDropDown)} className="dropdown-item text-sm py-2 px-4 font-normal block w-full 
                ...
                <div className={`flex w-full bg-[#11998E] ${showNavBar ? 'block md:hidden' : 'hidden'}`}>
                    <ul className="navbar-nav flex flex-col pl-0 gap-2 list-style-none mr-auto">
                        <li className="nav-item p-2">
                            <Link className="nav-link text-white" to="/">Home</Link>
                        </li>
                        {
